...
v1.0.9
- Improved Chrome Extension (Added Exclusion site list in options)

v1.0.8
- Improved Chrome Extension (Different Sized Icons, Options Page, Double-click disable/enable)
...
var ParallelUniverse = function(docObj) 
{
	...
	var childTags = new Array("A", "B", "BR", "EM", "I", "SMALL", "STRONG", "U");
	...
	function changeFont(docObject)
	{
		...
		for(var i = 0; i < htmlTags.length; i++) 
		{
			...
			else if(!htmlTags[i].hasChildNodes() && isExistInArray(htmlTags[i].tagName, childTags))
			{
				if (!isExistInArray(htmlTags[i].parentNode.tagName, excludedTags))
				{
					changeFontFamily(htmlTags[i].parentNode);
